...
class RespondentController extends Controller
{
    ...
    public function submitFinalAnswer(Request $request)
    {
        ...
        // Ambil email dari SESSION, bukan dari input request
        ...
        // Simpan atau update ke tabel master_respondent
        ...
        foreach ($semuaJawaban as $key => $value) {
            // Skip jika bukan input pertanyaan
            if (strpos($key, 'pertanyaan_') !== 0 || strpos($key, 'other_') !== false) {
                continue;
            }

            $pertanyaanId = str_replace('pertanyaan_', '', $key);

            // Cek apakah pilihan ganda (array)
            if (is_array($value)) {
                ...
                if (is_numeric($value)) {
                    // Jawaban numerik biasa (radio/checkbox)
                    AnswerSurvey::create([
                        'master_respondent_id'   => $respondentId,
                        'master_pertanyaan_id'   => $pertanyaanId,
                    ...
                    // Jawaban teks (text field)
                    if (!is_numeric($respondentId)) continue;

                    AnswerSurvey::create([
                        'master_respondent_id'   => $respondentId,
                        'master_pertanyaan_id'   => $pertanyaanId,
                        'pertanyaan_options_id'  => null,
                        'jawaban_teks'           => $value,
                        'lainnya'                => $semuaJawaban["lainnya_$pertanyaanId"] ?? null,
                    ]);
                }
        ...
        session()->forget([
            ...
            'form_pelayanan',
            'form_pengiriman',
            'email_respondent' // <- tambahkan ini agar tidak nyangkut
        ]);
