...
<body>
                ...
                <div class="row">
                    <div
                        ...
                        <div class="card card-primary">
                            ...
                            <div class="card-body">
                                @php
                                    $sessionData = session('form_kualitas', []);
                                @endphp

                                <form id="formSurveyKualitas" method="POST"
                                    action="{{ route('post-form-pertanyaan-kualitas') }}">
                                    @csrf

                                    @foreach ($pertanyaanFormKualitas as $pertanyaan)
                                    ...
                                    <div class="form-group d-flex justify-content-between">
                                        <a href="#" class="btn btn-outline-warning"
                                            onclick="document.getElementById('formSurveyKualitas').reset(); return false;">Clear
                                            Form</a>

                                        <div class="d-flex" style="gap: 10px;">
                                            <a href="{{ route('form-utama') }}" class="btn btn-light">Back</a>
    ...
    {{-- Script to toggle input 'Lainnya' --}}
    {{-- <script>
        function toggleOtherInput(input, id) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Show "Lainnya" inputs on page load if already selected
            const allRadios = document.querySelectorAll('input[type=radio]');
            const allCheckboxes = document.querySelectorAll('input[type=checkbox]');

            allRadios.forEach(radio => {
                if (radio.checked && radio.value.startsWith('other_')) {
                    const id = radio.name.split('_')[1];
                    const input = document.getElementById('other_input_' + id);
                    if (input) input.style.display = 'block';
                }
            });

            allCheckboxes.forEach(checkbox => {
                if (checkbox.checked && checkbox.value.startsWith('other_')) {
                    const name = checkbox.name; // pertanyaan_5[]
                    const match = name.match(/pertanyaan_(\d+)\[\]/);
                    const questionId = match ? match[1] : null;
                    const optionId = checkbox.value.split('_')[1];
                    const input = document.getElementById('other_input_' + questionId + '_' + optionId);
                    if (input) input.style.display = 'block';
                }
            });
        });
    ...
    <script>
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            document.querySelectorAll('input[name="required_checkbox[]"]').forEach(function(hiddenInput) {
                ...
                if (!checked) {
                    isValid = false;
                    alert('Mohon pilih minimal satu opsi pada pertanyaan wajib.');
                }
            });
