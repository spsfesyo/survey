...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasTable('plot_hadiah_survey')) {
            Schema::create('plot_hadiah_survey', function (Blueprint $table) {
                $table->id();
                ...
                $table->foreignId('master_outlet_survey_id')->nullable()
                    ->constrained('master_outlet_survey') // sesuai yang sudah kamu pakai
                    ->nullOnDelete()
                    ->cascadeOnUpdate();
                ...
                $table->enum('status_plot', ['Y', 'N'])
                    ->nullable()
                    ->default(null);

                $table->enum('status_respondent_assigned', ['Y', 'N'])
                    ->nullable()
                    ->default(null);
    ...
    {
         Schema::dropIfExists('plot_hadiah_survey');
    }
};
